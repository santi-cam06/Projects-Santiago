...
> [!NOTE]
The content inside `./src/advisor/user_jobpost_JSON/`, `./reports/` and `./data/` is Git ignored (except for `./data/jobs_postings/` which is the **database**) as it is private information that you should manage locally, you can change the `./.gitignore` file at your convenience.
...
- Summarizes the data in `./data/jobs_JSON/` which is parsed from real PDFs in `./data/jobs_postings/` plus RAG enriched to get extra information about each employer. 
...
## Main Application (end-user usage): Job Advisor
...
> [!NOTE]
**Cloud Workspace Users (Codespaces, GitPod, ...)**: If you are working in a remote environment you will need to upload the job posting PDF into `./data/user/` in the way you did for the resume (NOTE it's a different directory though). Then copy its absolute path and use it in the command above.
...
- **Codespaces**: `python3 -m src.advisor.app_advisor "/workspaces/Projects-Santiago/Job-Advisor/data/user/job_posting.pdf"`
...
#### What it does
- Generates a Market Analysis Report & Resume Comparison Report using the previous phases.
